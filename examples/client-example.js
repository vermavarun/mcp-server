...
function getFirstTextContent(content) {
    const textItem = content.find((item) => {
        if (!item || typeof item !== 'object')
            return false;
        const maybeContent = item;
        return maybeContent.type === 'text' && typeof maybeContent.text === 'string';
    });
    return textItem?.text ?? '';
}
function getFirstResourceTextContent(contents) {
...
async function main() {
    ...
    try {
        ...
        // Create some notes
        await client.createNote('Project Ideas', 'Build an MCP server for note management', ['work', 'programming']);
        await client.createNote('Grocery List', 'Milk, eggs, bread, coffee', ['personal']);
        await client.createNote('Learning Goals', 'Learn TypeScript, study MCP protocol, practice system design', ['education', 'programming']);
        // List all notes
